...
module.exports = function reactIntlPoLoader(source) {
  ...
  return Object.keys(parsed).reduce(
    (result, key) => {
      ...
      const [msgCtxt, defaultMessage] = key.split('\u0004');

      if (msgCtxt) {
        // The value is a array with 2 elements, the first one always being null since react-intl
        // handles plurals with ICU message format, not msgid_plural.
        // If useDefaultMessage is true and a translation is missing, use the defaultMessage.
        result[msgCtxt] = parsed[key][1] || (options.useDefaultMessage ? defaultMessage : '');
      }
